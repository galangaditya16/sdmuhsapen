...
@section('content')
    <div class="col-md-12">
        <form class="card" method="POST" action="{{ route('gallery.update',$data) }}" enctype="multipart/form-data">
            @csrf @method('PUT')
            <div class="card-header">
                <h3 class="card-title">Edit Images Asset</h3>
            </div>
            <div class="card-body">
                <div class="mb-3">
                    <label class="form-label required">Nama(ID)</label>
                    <div>
                        <input type="text" class="form-control" name="title_id" aria-describedby="title" placeholder="Enter Name ID" value="{{ $data->title_id ?? old('name_id')}}">
                        @error('menu_name')
                ...
                <div class="mb-3">
                  ...
                  <div>
                      <input type="text" class="form-control" name="title_en" aria-describedby="title" placeholder="Enter Name EN" value="{{ $data->title_en ?? old('name_en')}}">
                      @error('menu_name')
                          <div class="invalid-feedback" style="display: block">{{ $message }}</div>
                      @enderror
                  </div>
