...
@section('content')
  ...
  <section class="container mx-auto grid grid-cols-1 gap-y-7 md:my-20">
    ...
    <div class="px-4 lg:px-0 w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      @foreach ($galeries as $new)
        <div style="background-image: url('{{ $new['bg-image'] }}');" alt="galeries-1.png" class="rounded-3xl bg-cover bg-center h-[565px] max-h-[600px] w-full relative overflow-hidden">
          <div class="h-[565px] relative">
            <div class="py-1.5 px-4 bg-black bg-opacity-40 text-white rounded-lg absolute left-8 top-[235px] flex gap-x-1">
          ...
          <div class="absolute bottom-0 bg-white pt-6 px-9 h-1/2 w-full">
            ...
            <div class="flex justify-center mt-5">
              <a href="{{ route('galeryDetail', str($new['title'])->slug()) }}" class="py-2 px-10 bg-oren text-white rounded-3xl">Lihat Galeri</a>
            </div>
          </div>
        </div>
      @endforeach
