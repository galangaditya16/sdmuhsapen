...
@section('content')
    ...
    <section class="container mx-auto md:my-20 relative">
        <p class="px-4 md:px-0 font-bold mb-6">{{ __('message.hasil_penelusuran_berita') }} "{{ request('search') ?? '-' }}"
        </p>
                ...
                @forelse ($news as $new)
                    ...
                    <div
                        ...
                        <div class="relative h-[300px]">
                            <img src="{{ $firstImage ? asset('assets/images/news') . '/' . $firstImage : asset('assets/images/no-image.png') }}" alt="{{ $new['title'] }}"
    ...
    <!-- images gallery -->
    <section class="container mx-auto md:my-20 relative">
        <p class="px-4 md:px-0 font-bold mb-6">{{ __('message.hasil_penelusuran_galeri') }}
            "{{ request('search') ?? '-' }}"</p>
        <button id="scrollLeft"
            class="absolute left-2 top-[50%] -translate-y-1/2 bg-biru-tua hover:bg-gray-300 text-black rounded-full p-2 z-20">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
        ...
        <div class="px-4 lg:px-4 w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @forelse ($galeris as $new)
                <div style="background-image: url('{{ asset('assets/images/gallery/thumbnail/') . '/' . $new->thumbnail }}');"
                    alt="galeries-1.png"
