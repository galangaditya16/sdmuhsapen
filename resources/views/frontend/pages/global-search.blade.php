...
@section('content')
                ...
                @forelse ($lists as $new)
                    @php
                        if (isset($new['content_news']) && $new['content_news']) {
                            $decode =
                                $new['content_news'] && $new['content_news']['images']
                                    ? json_decode($new['content_news']['images'])
                                    : '';
    ...
    <!-- images gallery -->
    <section class="container mx-auto md:my-20 relative">
      <p class="px-4 md:px-0 font-bold mb-6">Hasil penelusuran untuk "{{ request('search') ?? '-' }}"</p>
